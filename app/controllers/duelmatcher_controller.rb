		...
		if gameOngoing == nil
			...
			if BaseStat.find_by_account(session[:account]) != nil
				...
				if BaseStat.find_by_account(session[:account]).game != nil
					...
					GameStat.create(:account => session[:account], :game => @gameNumber, :room => 1, :character => @yourHero, :hp => @yourHero_hp, :maxhp => @yourHero_maxhp, :shield => 0, :mp => @yourHero_mp, :maxmp => @yourHero_maxmp, :pos => 0, :kills => 0, :deaths => 0, :exp => 0, :allies => "black")
			...
			if @gameNumber == nil
				...
				if otherAccount != false && BaseStat.find_by_account(otherAccount).character != params[:character]
					...
					if validHeroes.include? params[:character]
						...
						GameStat.create(:account => session[:account], :game => @gameNumber, :room => 1, :character => @yourHero, :hp => @yourHero_hp, :maxhp => @yourHero_maxhp, :shield => 0, :mp => @yourHero_mp, :maxmp => @yourHero_maxmp, :pos => 0, :kills => 0, :deaths => 0, :exp => 0, :allies => "white")
